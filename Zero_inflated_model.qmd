...
Daugumoje žuvininkystės duomenų rinkinių (mokslinių, verslinės ar megėjiškos žvejybos) yra labai daug nulinių reikšmių - kuomet buvo žvejota, bet nieko nepagauta. Šie nuliniai sugavimai yra svarbi duomenų dalis, tačiau tam modeliai turi būti specialiai pritaikyti. Čia pristatome modelį duomenims su nulinių reikšmių perviršiu. Modelis parašytas [R](https://www.r-project.org/) kalba, todėl jums reikės R ir [RStudio](https://www.rstudio.com/products/rstudio/). Jei jums reikia trumpo įvado apie R, peržiūrėkite [šį įvadinį puslapį](https://course.mizer.sizespectrum.org/prepare/install-tools.html) iš mūsų kurso apie dydžiu pagrįstą modeliavimą.

Įvadinę informaciją apie modelį ir duomenis galite rasti [**čia**](resources/ZeroInflatedModels.pdf). Tačiau jei norite naudoti modelį, primygtinai rekomenduojame atlikti bent [**4**](https://fishsizeproject.github.io/CPUEcourse/4-hilsha.html) mūsų CPUE standartizacijos kurso dalį, kur modelis pristatytas išsamiau. Be to, prieš pradėdami standartizuoti CPUE, įsitikinkite, kad atidžiai *įvertinote savo duomenis*. Tam paruošėme du kursus ir kodus, kuriuos rasite mūsų [svetainėje](https://fishsizeproject.github.io/CPUEcourse/). 
